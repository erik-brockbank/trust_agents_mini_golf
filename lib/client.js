    ...
    $("body").load(HTML_LOOKUP["survey_round"], function() {
        ...
        $('input[type="range"][id="eval-slider"]').on('input', function(e) {
            ...
            if (canvas.getContext) {
                ...
                draw_slider_context();
                // draw new circle
                // this ensures that at max slider val, radius is "epsilon" (5) px. from edge and at min slider, it's epsilon greater than 0
                var newRad = BINHEIGHT/2 - (sliderVal * (BINHEIGHT/14)) + 5 // TODO get max. slider val here rather than hard coding 2*7
...
draw_slider_context = function() {
    ...
    if (canvas.getContext) {
        ...
        ctx.fill();
        // initial ring around target
        var sliderVal = $('input[type="range"][id="eval-slider"]').val();
        var initRad = BINHEIGHT/2 - (sliderVal * (BINHEIGHT/14)) + 5 // TODO get max. slider val here rather than hard coding 2*7
        ctx.strokeStyle = "rgb(255, 0, 0, 0.5)";
        ctx.lineWidth = 5;
        ctx.beginPath(); // NB: this is necessary to avoid lines across radius of circles
        ctx.arc(BIN_STARTX + BINWIDTH/2, BIN_STARTY + BINHEIGHT/2, initRad, 0, Math.PI * 2, false); // center x, center y, radius, starting angle (rads), ending angle
        ctx.stroke();
        ctx.closePath();
    }
};
